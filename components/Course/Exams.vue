              ...
              <tr v-for="(item, index) in examQuestions" :key="index">
                ...
                <td class="col-2">
                  ...
                  <button
                    ...
                    v-on:click="openExam('edit', item.id)"
                  >Sửa</button>
                </td>
              </tr>
            </draggable>
...
export default {
  ...
  mounted: async function () {
    this.examQuestions = await this.getExamQuestions(this.courseId);
  },
  methods: {
    ...
    async getExamQuestions(courseId) {
      try {
        let result = await this.$axios.get("/api/course/" + courseId + "/examquestions");
        if (result.status === 200) {
          return result.data;
        }
      } catch (e) {
